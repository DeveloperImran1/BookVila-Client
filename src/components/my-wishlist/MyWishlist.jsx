"use client";
import { favoruteBookGet, removeBook, setCartBook } from "@/hooks/localStorage";
  ...
  return (
    <div className="bg-white p-4 ">
      <p className="text-[17px] font-semibold mb-6">
        My Total Favorute Books: {data?.length || 0}
      </p>
      {data?.length < 1 && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {data?.map((book) => (
          <div
            key={book?._id}
            className="w-full max-w-[340px] space-y-3  bg-white p-4 shadow-lg  border-2 rounded"
          >
            <div className="relative flex h-[250px] w-full justify-center ">
              <div className="absolute left-2 w-full flex items-center justify-between">
                {book?.books?.discount > 0 && (
                  <button className="rounded-xl bg-[#0095FF] px-3 py-1 font-medium text-white duration-200 hover:bg-[#0095FF]/90">
                    {book?.books?.discount}% off
                  </button>
                )}

                <MdDelete
                  onClick={() => handleDeleteWishlist(book)}
                  size={23}
                  className=" text-primary hover:text-red-500 hover:scale-125 cursor-pointer duration-300 "
                ></MdDelete>
              </div>
              ...
              <Image
                width={400}
                height={400}
                className="rounded-lg bg-white m-auto mt-9 h-[200px] w-[150px] "
                src={
                  book?.books?.coverImage ||
